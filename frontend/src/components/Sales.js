...
const Sales = () => {
  ...
  const fetchData = async () => {
    ...
    setTimeout(() => setDelay(false), 100); // Introduce a delay of 1 second before displaying the content
  };
  ...
  const groupDataByCity = (data) => {
    return data.reduce((acc, curr) => {
      const city = curr.productId?.category; // Assuming 'city' field is present in the data

      if (!acc[city]) {
        acc[city] = { city: city, count: 0 };
      }
  ...
  console.log(groupDataByCity(data1));
  ...
  const transformedData = Object.entries(groupDataByCity(data1)).map(([date, { count }]) => ({
    name: date,
    users: count,
  }));
  ...
  return (
    <div>
      <div className='bg-white py-2 mt-1 px-4 rounded-lg flex justify-between items-center customShadow1'>
        <h2 className='font-bold text-lg'>Analysis of Category wise Orders</h2>
      </div>
      <div className='flex justify-between w-full px-5'>
        <aside className='ml-2 mt-[10px] bg-white rounded-lg p-2 h-[100px] w-[190px] customShadow'>
          <div>
            <div className=''>
              <h3 className='text-base uppercase font-medium text-slate-500 border-b pb-1 border-slate-300'>Category</h3>
              <form className='text-sm flex flex-col gap-2 py-2'>
                {
                  productCategory.slice(0, 1).map((categoryName, index) => (
          ...
          {selectCategory && (
            <div>
              <div className='py-2 mt-1 px-4 flex justify-center items-center'>
                <h1 className='font-bold text-lg capitalize'>Category vs Order</h1>
              </div>
